...
def _get_gapdays(df, key):
    ...
    df['{}_gapdays'.format(key)] = date_object_delta / np.timedelta64(1, 'D')
    df.loc[pd.isnull(df['{}_gapdays'.format(key)]), '{}_gapdays'.format(key)] = 1
    new_key = '{}_gapdays'.format(key)

    grouped_df = df.groupby([key, 'date'])[new_key].agg(np.max).to_frame(name='uniform_gapday')
    grouped_df.reset_index(inplace=True)
    df = pd.merge(df, grouped_df, how='left', on=[key, 'date'])
    df[new_key] = df['uniform_gapday']
    return df
...
def _columns_to_write():
    return ['projectid', 'schoolid_gapdays', 'schoolid_cost_gap',
            'school_city_gapdays', 'cost_by_school_city_cummax', 'cost_by_school_zip_cummin', 'tot_cost_per_day_schoolid']
...
if __name__ == '__main__':
    ...
    df1['schoolid_cost_gap'] = df1['total_price_including_optional_support'] - df1['price_prev']
    df1.loc[pd.isnull(df1['schoolid_cost_gap']), 'schoolid_cost_gap'] = 1

    df1 = df1[['projectid', 'schoolid_gapdays', 'schoolid_cost_gap']]
    ...
    df2 = df2[['projectid', 'school_city_gapdays', 'cost_by_school_city_cummax']]
    ...
    df3['price_school_zip_cummin'] = df3.groupby('school_zip')['price_prev'].cummin()
    df3.loc[df3['price_school_zip_cummin'] == 99999999, 'price_school_zip_cummin'] = np.nan

    df3 = df3[['projectid', 'price_school_zip_cummin']]
    ...
    projects_df['cost_by_school_zip_cummin'] = projects_df['price_school_zip_cummin'] - projects_df[
    ...
    test_df = pd.merge(outputs_df, projects_df[['projectid', 'date_posted', 'schoolid', 'school_city', 'school_zip']], how='left', on='projectid')

    test_df[['projectid', 'date_posted', 'schoolid','schoolid_gapdays', 'schoolid_cost_gap', 'tot_cost_per_day_schoolid']].\
        sort(['schoolid', 'date_posted'])[:50000].to_csv(os.path.join('../FeaturesValidations', 'prev_comprisons_school.csv'))
    test_df[['projectid', 'school_city', 'date_posted', 'school_city_gapdays', 'cost_by_school_city_cummax']].\
        sort(['school_city', 'date_posted'])[:50000].to_csv(os.path.join('../FeaturesValidations', 'prev_comprisons_city.csv'))
    test_df[['projectid', 'school_zip', 'date_posted', 'cost_by_school_zip_cummin']].\
        sort(['school_zip', 'date_posted'])[:50000].to_csv(os.path.join('../FeaturesValidations', 'prev_comprisons_zip.csv'))
